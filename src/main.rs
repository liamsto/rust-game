...
fn main() {
    ...
    let player = character::Character {
        ...
        level: 1,
        moves: [
            Some(all_moves::PUNCH.clone().into()),
            Some(all_moves::DISMANTLE.clone().into()),
            Some(all_moves::FLAMETHROWER.clone().into()),
            None,
        ],
        ...
        experience: 0.0,
        known_moves: vec![
            all_moves::PUNCH.clone().into(),
            all_moves::DISMANTLE.clone().into(),
            all_moves::FLAMETHROWER.clone().into(),
        ],
    };

    let enemies: Vec<Arc<Mutex<dyn Combatant>>> =
